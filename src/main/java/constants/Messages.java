package constants;

/**
 * Created by Ярослав on 21.04.2017.
 */
public interface Messages {

    String MESSAGE_403 = "Error 403! You haven't access to this resource.";
    String MESSAGE_404 = "Error 404! Incorrect resource location";
    String ON_INCORRECT_LOGIN_MESSAGE = "Incorrect login or password";
    String ON_LOGIN_OCCUPIED_MESSAGE = "Selected login already in use!";
    String INCORRECT_PROFESSOR_LOGIN_MESSAGE = "There is no professor with such login. Check users list.";
    String INCORRECT_INPUT_DATA_MESSAGE = "Incorrect input data";
}
